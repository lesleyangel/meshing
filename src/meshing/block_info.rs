...
pub struct BlockInfo {
    pub eta_delta: f32, // eta方向结构间隔
    pub eta_num: usize, // eta方向每个block间网格数
    pub fai_delta: f32, // fai方向结构间隔
...
impl BlockInfo {
    pub fn calc_info(&self) -> Option<MeshMat> {
        ...
        let eta_mesh_delta = self.eta_delta / self.eta_num as f32; //每个block中网格间距
        let fai_mesh_delta = self.fai_delta / self.fai_num as f32; //每个block中网格间距
        let eta_margin_size = ((1. - eta_size as f32 * self.eta_delta) / eta_mesh_delta) as usize;
        let fai_margin_size = ((1. - fai_size as f32 * self.fai_delta) / fai_mesh_delta) as usize;
        let eta_margin_delta = (1. - eta_size as f32 * self.eta_delta) / eta_margin_size as f32;
        let fai_margin_delta = (1. - fai_size as f32 * self.fai_delta) / fai_margin_size as f32;
